...
<template>
  <div class="h-screen flex flex-col md:flex-row bg-bg overflow-hidden">
    <!-- ── Mobile top bar ── -->
    <header
      class="md:hidden h-14 shrink-0 flex items-center gap-2 px-3 border-b border-border-soft bg-bg/90 backdrop-blur z-30"
    >
      <button
        class="btn btn-sm btn-ghost !px-2 !h-9 !w-9"
        title="打开菜单"
        @click="openMobileNav"
      >
      ...
      >
        ...
        <span v-if="mobileSubTitle" class="text-text-mute font-normal text-xs">
          {{ mobileSubTitle }}
        </span>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
             stroke="currentColor" stroke-width="2.5" stroke-linecap="round"
             stroke-linejoin="round" class="shrink-0 text-text-mute">
          <path d="M6 9l6 6 6-6" />
        </svg>
      </button>
      <div
      ...
      <button
        ...
        @click="toggleTheme"
      >
        <svg v-if="themeState.mode === 'dark'" width="16" height="16"
    ...
    <!-- ── Main nav sidebar (chat-ui style: 渐变背景 + 圆角右侧) ── -->
    <aside
      class="flex flex-col shrink-0 transition-[width,transform] duration-300 ease-out
        max-md:fixed max-md:top-0 max-md:left-0 max-md:bottom-0 max-md:w-[260px] max-md:z-50
        max-md:rounded-r-2xl max-md:border-r max-md:border-border-soft max-md:shadow-2xl
        md:static"
      :class="[
    ...
    >
      <!-- Logo header -->
      <div
        class="flex items-center gap-2.5 pt-4 pb-4 select-none"
        :class="collapsed ? 'md:px-3 md:justify-center' : 'px-4'"
      >
        <div
        ...
        <div v-if="!collapsed" class="flex flex-col leading-tight min-w-0">
          <strong class="text-base font-semibold tracking-tight text-text truncate">
            AGENT
          </strong>
      ...
      <!-- Nav items -->
      <div class="flex flex-col gap-0.5 px-2 flex-1 min-h-0 overflow-y-auto">
        <div
          v-if="!collapsed"
          class="text-xxs uppercase tracking-wider text-text-faint px-3 pt-2 pb-1.5 font-semibold"
        >
          工作区
        </div>
        <router-link
        ...
        >
          <button
            ...
            @click="navigate(); closeMobileNav();"
          >
            <span class="w-4 h-4 grid place-items-center shrink-0" aria-hidden="true">
              <svg v-if="item.icon === 'chat'" width="16" height="16"
                   viewBox="0 0 24 24" fill="none" stroke="currentColor"
                   stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
              </svg>
              <svg v-else-if="item.icon === 'tasks'" width="16" height="16"
                   viewBox="0 0 24 24" fill="none" stroke="currentColor"
                   stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                <path d="M9 11l3 3L22 4" />
                <path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11" />
              </svg>
              <svg v-else-if="item.icon === 'memos'" width="16" height="16"
                   viewBox="0 0 24 24" fill="none" stroke="currentColor"
                   stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
              ...
              <svg v-else-if="item.icon === 'memories'" width="16" height="16"
                   viewBox="0 0 24 24" fill="none" stroke="currentColor"
                   stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
            ...
            <span v-if="!collapsed" class="flex-1 min-w-0 truncate">
              {{ item.label }}
            </span>
          </button>
        </router-link>
      ...
      <!-- Desktop footer: theme + collapse toggle -->
      <div
        class="hidden md:flex items-center px-2 py-2.5 border-t border-border-soft gap-1"
        :class="collapsed ? 'justify-center flex-col gap-1' : 'justify-between'"
      >
        <button
          ...
          @click="toggleTheme"
        >
          <svg v-if="themeState.mode === 'dark'" width="14" height="14"
               viewBox="0 0 24 24" fill="none" stroke="currentColor"
               stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <circle cx="12" cy="12" r="4" />
            <path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41" />
    ...
    <!-- Main content -->
    <main class="flex-1 flex flex-col min-w-0 min-h-0 bg-bg overflow-hidden">
