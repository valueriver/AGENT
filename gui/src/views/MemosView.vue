    ...
    <div class="mx-auto max-w-3xl px-5 py-6 flex flex-col gap-5">
      <header class="flex items-center justify-between gap-3 flex-wrap">
        <div class="flex items-baseline gap-3 min-w-0">
          <h2 class="m-0 text-2xl font-semibold text-text leading-tight">便签</h2>
          <p class="m-0 text-sm text-text-mute">
            跨会话时间轴 · 共 {{ memos.length }} 条
          </p>
        </div>
      ...
      <div v-else class="flex flex-col gap-8">
        <section v-for="g in groups" :key="g.key" class="flex flex-col gap-3">
          <div
            class="sticky top-0 z-10 -mx-5 px-5 py-1.5 bg-bg/95 backdrop-blur
          ...
          <ol class="relative flex flex-col gap-4 pl-6">
            <span
              class="absolute left-[9px] top-2 bottom-2 w-px bg-border-soft"
              aria-hidden="true"
            ></span>
            <li v-for="m in g.items" :key="m.id" class="relative">
              <span
                class="absolute -left-[19px] top-[7px] w-[10px] h-[10px] rounded-full ring-4 ring-bg"
                style="background: var(--color-accent);"
                aria-hidden="true"
              ></span>
              <div class="flex flex-col gap-1.5 py-0.5">
                <div class="flex items-baseline gap-2 text-xxs text-text-faint flex-wrap">
